...
if "--" not in sys.argv:
    ...
    print("blender --background --python exportAnimationSC.py -- [--reverse] input.blend output.MOT")
    sys.exit(1)
...
print(f"Will export SC animation to: {output_path}")
...
# ======================  EXPORT ======================
def export_saturn_animation_sc_binary(output_path):
    ...
    def process_bone(bone, accum_mat=Matrix.Identity(4), is_root_like=False, use_pose=False, compute_output=False, build_structure=False):
        ...
        # Build rotation matrix (exact same as your working print script)
        ...
        if has_mesh and compute_output:
            ...
            # Exact sin/cos you used in the working script
            sx = math.sin(euler_s.x)
            cx = math.cos(euler_s.x)
            sy = math.sin(-euler_s.y)
            cy = math.cos(-euler_s.y)
            sz = math.sin(-euler_s.z)
            cz = math.cos(-euler_s.z)

            current_frame_data.append([
                to_fixed(pos_s[0]),
                to_fixed(pos_s[1]),
                to_fixed(pos_s[2]),
                to_fixed(sx), to_fixed(cx),
                to_fixed(sy), to_fixed(cy),
                to_fixed(sz), to_fixed(cz),
    ...
    print(f"Exporting SC {num_frames} pose(s) × {num_transforms} transform(s)")

    # Write binary .MOT (same header format as your angle version)
    with open(output_path, 'wb') as f:
        f.write(struct.pack('>II', 1, 1))                    # version, animationCount=1
        f.write(struct.pack('>hh', 1, num_frames))           # spec
        ...
        for fnum in frame_range:
            ...
            # Fill popCount (Uint32)
            for i, t in enumerate(current_frame_data):
                t[9] = pop_counts[outputted_bones[i]]   # index 9 = popCount

            # Write one TransformSC: 3×FIXED + 6×FIXED + Uint32 pop = 40 bytes
            for t in current_frame_data:
                f.write(struct.pack('>iiiiiiiiiI', *t))

    print(f"✅ SC binary animation exported: {output_path}")
    print(f"    Poses     : {num_frames}")
    print(f"    Transforms: {num_transforms}")
    print(f"    Size      : {8 + 4 + num_frames * num_transforms * 40} bytes")

# ======================  RUN ======================
export_saturn_animation_sc_binary(output_path)
