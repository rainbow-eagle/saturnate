...
uint32_t nextFreeCGBlockIndex = 0;

TEXTURE texturePool[MAX_POLYGON_COUNT];
PDATA meshPool[MAX_MESH_COUNT] __attribute__((aligned(4)));
PDATA* nextFreeMesh;
uint32_t textureCountByModelPool[MAX_MESH_COUNT] __attribute__((aligned(4))); // same 100 than meshPool
...
int32_t ttLoadNYAMeshesInPool(char* fileName) {
    ...
    if(nextFreeTextureCountByModel > textureCountByModelPool)
        for(uint32_t* textureCountByModel = nextFreeTextureCountByModel - 1; textureCountByModel >= textureCountByModelPool; textureCountByModel--)
            textureOffset += *textureCountByModel;
    int32_t meshCount = ttLoadNYAMeshes(fileName, nextFreeMesh, nextFreePoint, nextFreePolygon, nextFreeAttribute,
