...
typedef struct {
    FIXED xOffset, yOffset, zOffset; /** x, y and z offset to be applied during a position shift (slTranslate()). */
    ANGLE xAngle, yAngle, zAngle; /** x, y and z angle to be applied during rotations (calling in order slRotX, slRotY, slRotZ). */
    uint16_t popCount; /** Number of pops to be applied after applying the transformations. 0 to not pop anything. */
} TTTransform;
 ...
 */
typedef struct {
    uint8_t reservedSpace1; /** Should be a file format descriptor to make sure you're reading the right type of file */
    uint8_t reservedSpace2; /** Should become a version number to allow retrocompatibility in case of evolution */
...
typedef struct {
    ...
    uint16_t transformOffset; /** Index (in the list of transforms of this animationSet, not globally) of the first TTTransform that describes this animation. */
} TTAnimationSpec;
 ...
 */
typedef struct {
    int8_t totalFrameCountBetweenPoses; /** (Copy of corresponding animationSpec data) Number of frames a pose remains in place before switching to the next pose in the animation. */
    int8_t totalPoseCount; /** (Copy of corresponding animationSpec data) Total number of poses that compose this animation. */
    int8_t currentPoseID; /** Index of the pose currently assumed by the model (between animationSpec totalPoseCount-1 and zero). */
    int8_t frameCountBeforeNextPose; /** Remaining number of frames to wait before switching to the next pose. */
} TTAnimationState;
...
// void loadAnimationSC1(char* fileName, TTAnimationSpec* animationSpecs, uint32_t meshCount, TTTransformSC animationTransforms[][meshCount]);
 ...
 */
static inline __attribute__((always_inline)) void ttDrawAnimatedModelAtState(PDATA model[], TTTransform* animation, uint32_t meshCount, TTAnimationState* animationState) {
 ...
 * @param[in] animationState The current state of the animation.
 */
static inline __attribute__((always_inline)) void ttDrawAnimatedModelFromSpecs(PDATA model[], TTTransform* animations, uint32_t meshCount, TTAnimationSpec* animationSpec, TTAnimationState* animationState) {
