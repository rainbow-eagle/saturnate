...
void ttLoadAnimation(char* fileName, TTMOTFileHeader* animationSetSpec, TTAnimationSpec* animationSpecs, TTTransform* animations) {
    ...
    uint32_t size = sizeof(TTAnimationSpec) * animationCount;
    ...
    ttReadBytes(&stream, size, (uint8_t*) animations);
...
//     uint32_t meshCount = fileHeader->meshCount;
//     char* bufferPointer = TO_UNCACHED_ADDR(loadedPool) + sizeof(TTMOTFileHeader);
//     uint32_t size = sizeof(TTAnimationSpec) * fileHeader->animationCount;
