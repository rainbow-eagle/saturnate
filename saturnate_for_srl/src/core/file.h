 ...
 */
typedef struct {
    int32_t tmpBufferedByteCount; /**Number of bytes currently buffered (sector size - number of already read bytes)*/
    GfsHn fileHandle; /**GFS file handle to the opened file*/
    uint8_t tmpBuffer[TT_SECTOR_SIZE]; /**Buffer where the lastly read sector is stored*/
} TTFileStream;
