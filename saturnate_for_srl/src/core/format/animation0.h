...
typedef struct {
    ...
    uint32_t meshCount; ///< [in] The number of meshes in the model (also is the number of transformations in the pose).
} TTPoseDescription0;
...
typedef struct {
    ...
    uint32_t maxAnimationSpecCount; ///< [in] Maximum amount of TTAnimationSpec0 that can be added to animationSpecs.
    TTTransform* animations; ///< [out] Array where the animations' TTTransform are loaded.
    uint32_t maxTransformCount; ///< [in] Maximum amount of TTTransform that can be added to animations.
} TTAnimationBuffer0;
 ...
 */
static inline __attribute__((always_inline)) void ttDrawAnimatedModelAtState0(PDATA model[], TTTransform* animation, uint32_t meshCount, TTAnimationState* animationState) {
 ...
 * @param[in] animationState The current state of the animation.
 */
static inline __attribute__((always_inline)) void ttDrawAnimatedModelFromSpecs0(PDATA model[], TTTransform* animations, uint32_t meshCount, TTAnimationSpec0* animationSpec, TTAnimationState* animationState) {
