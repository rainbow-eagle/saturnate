 ...
 */
typedef struct {
    Uint8* meshCountPtr; ///< [out] Pointer to the total number of loaded meshes.
    PDATA* meshes; ///< [out] Array of PDATA where meshes are to be stored (one per mesh).
    Uint32 maxMeshCount; ///< [in] Maximum amount of PDATA that can be added to meshes.
    POINT* points; ///< [out] Vertex array (POINT) to store all mesh coordinates.
    Uint32 maxPointCount; ///< [in] Maximum amount of POINT that can be added to points.
    POLYGON* polygons; ///< [out] Polygon array (POLYGON) defining face indices.
    Uint32 maxPolygonCount; ///< [in] Maximum amount of POLYGON that can be added to polygons.
    ...
    Uint32 maxTextureCount; ///< [in] Maximum amount of TEXTURE that can be added to textures.
    Uint32 textureOffset; ///< [in] Index of the next free texture slot in SGL's texture pool (the one expected by slInitSystem).
    Uint32* textureCountPtr; ///< [out] Pointer to the total number of loaded textures.
    Uint32* textureDataAddrPtr; ///< [out] Pointer to the where the texture data (actual pixels) is to be stored in VRAM.
} TTMeshBuffer;
 ...
 */
typedef struct {
    Uint32 pointCount; ///< Total number of vertices in the file.
    Uint32 polygonCount; ///< Total number of polygons in the file.
    Uint32 textureCount; ///< Total number of textures in the file.
    Uint16 vramCGCount; ///< Total number of VRAM cells that would be used by the textures in the file.
    Uint8 meshCount; ///< Total number of meshes in the file.
    Uint8 reservedSpace;
} TTNYAFileMetadata;
