...
#define TT_MASTER_CPU_STACK_END AdjWork(CommandBuf, sizeof(Uint32) * 8, MaxPolygons)
 ...
 * @par Example:
 * @code
 * Uint8* nextFree = (Uint8*) TT_LRAM_HEAP_START; // Start of Low RAM
 * 
 * // Allocate 400 bytes for integers (aligned to 4 bytes)
 * Uint32* myHundredIntegers = TT_LALLOC(nextFree, 100 * sizeof(Uint32));
 * 
...
#define TT_LALLOC(nextFreeByte, size) \
    ({ \
        Uint8* __aligned = (Uint8*)(((Uint32)(nextFreeByte) + 3) & ~3); \
