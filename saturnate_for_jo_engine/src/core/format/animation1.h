...
typedef struct {
    ...
    Uint8 popCount; ///< Number of matrix pops to be applied after applying the animation transformations for this bone. Used to characterize the hierarchy within the armature. 0 to not pop anything.
    Uint8 reservedSpace1;
    Uint16 reservedSpace2;
...
typedef struct {
    ...
    Uint16 poseOffset; ///< Index (in the list of poses of this animationSet, not globally) of the first pose that describes this animation, i.e. sum of all the totalPoseCount that precede it it.
} TTAnimationSpec1;
...
typedef struct {
    ...
    Uint32 meshCount; ///< [in] The number of meshes in the model (which is also the number of bones in the armature and boneRotations in the pose)
} TTPoseDescription1;
...
typedef struct {
    ...
    Uint32 maxAnimationSpecCount; ///< [in] Maximum amount of TTAnimationSpec1 that can be added to animationSpecs.
    Uint32 maxBoneCount; ///< [in] Maximum amount of TTBone that can be added to armature.
    Uint32 maxRootBonePositionByPoseCount; ///< [in] Maximum amount of TTRootBonePosition that can be added to rootBonePositionByPose.
    Uint32 maxTransformCount; ///< [in] Maximum amount of TTBoneRotation that can be added to animations.
} TTAnimationBuffer1;
 ...
 */
static inline __attribute__((always_inline)) void ttDrawAnimatedModelAtState1(PDATA* model, TTBone* armature, TTBoneRotation* animation, TTRootBonePosition* animationRootBonePositions, Uint32 meshCount, TTAnimationState* restrict animationState) {
 ...
 * @param[in] animationState The current state of the animation.
 */
static inline __attribute__((always_inline)) void ttDrawAnimatedModelFromSpecs1(PDATA model[], TTBone* armature, TTBoneRotation* animations, TTRootBonePosition* rootBonePositions, Uint32 meshCount, TTAnimationSpec1* restrict animationSpec, TTAnimationState* animationState) {
