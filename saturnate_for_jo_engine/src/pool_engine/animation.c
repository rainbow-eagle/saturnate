...
Uint8 animationSpecOffsetByModelPool[MAX_ANIMATED_MODEL_COUNT] __attribute__((aligned(4))); //offset[modelID] = ID du premier animationSpec pour ce modèle
Uint8* nextFreeAnimationSpecOffsetByModel = animationSpecOffsetByModelPool;
...
Uint8 animatedInstanceCountByModelPool[MAX_ANIMATED_MODEL_COUNT] __attribute__((aligned(4))); // same 20 than animationSetSpecPool, the above like and the next line
Uint8 animatedInstanceOffsetByModelPool[MAX_ANIMATED_MODEL_COUNT] __attribute__((aligned(4))); // offset[modelID] = offset[modelID-1] + count[modelID-1]. offset[modelID] = ID de la première instance pour ce modèle
...
void ttSetAnimatedInstanceCountInPool(Uint8 animatedModelCount, Uint8 instanceCounts[animatedModelCount]) {
    ...
    Uint8* instanceCountPtr = animatedInstanceCountByModelPool;
    Uint8* instanceOffsetPtr = animatedInstanceOffsetByModelPool;
...
void ttStartAnimationInPool(Uint8 modelID, Uint8 instanceID, Uint8 animationSpecID) {
    Uint32 specOffset = animationSpecOffsetByModelPool[modelID];
    Uint32 instanceOffset = animatedInstanceOffsetByModelPool[modelID];
...
void ttUpdateAnimationsInPool() {
    ...
    const Uint8* restrict instanceCount = animatedInstanceCountByModelPool;
    TTAnimationState* animationStatePtr = animationStatePool;
    TTTransform** currentPosePtr = animatedInstanceCurrentPosePool;
